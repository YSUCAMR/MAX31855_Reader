...
## Delete text file before collecting new data

infile = open("/home/pi/evan/programs/pythermocouple/pi3n6/cs1data.txt","a") ##Avoids loss of data by appending to existing file 


##infile = open("/home/pi/evan/programs/pythermocouple/pi3n6/cs1data.txt","w+") ##Creates new file to write to if no existing
...
while True:
        ...
        if i < 15:
                ...
                i = 0
                average = mean(times[:])
                a = '{0}\t{1}\t{2}\t{3}\t{4}\n'.format( average, np.nanmean(alldata[:,0]),np.nanmean(alldata[:,1]), np.nanmean(alldata[:,2]),np.nanmean(alldata[:,3]))
                #print a
                times = []
                alldata = np.zeros((15,4))
                alldata[:] = np.NAN
                infile.write(a)
